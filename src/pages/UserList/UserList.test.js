...
const usersMock = [{ id: 0, name: 'Teste da Silva'}];
...
fdescribe('<UserList />', () => {
  ...
  it('Should redirect to user update route when user click on update button', async () => {
    ...
    const userSelected = await screen.findByText(usersMock[0].name);
    userEvent.click(userSelected);
    userEvent.click(updateBtn);

    expect(history.location.pathname).toBe('/alteracao-usuario/'+usersMock[0].id);
  });
